<x-app-layout>
    ...
    {{-- 移植するスタイル（Bladeのコメント構文に修正） --}}
    <style>
        .weather-card { width: 100%; max-width: 500px; margin: 20px auto; padding: 30px; border-radius: 8px; /* Breezeのスタイルに合わせる */ }
        .weather-card h1 { color: inherit; margin-top: 0; text-align: center; font-size: 1.5rem; }
        .weather-card h2 { border-bottom: 2px solid #5a5a5a; padding-bottom: 10px; text-align: center; font-size: 1.25rem;}
        .weather-card h3 { margin-top: 30px; border-bottom: 1px solid #4A5568; padding-bottom: 5px; text-align: left;}
        .weather-card p { margin: 12px 0; text-align: left; font-size: 1.1em;}
        .weather-card strong { font-size: 1.2em; color: #d9534f; float: right; }
        .weather-card hr { border: none; border-top: 1px solid #4A5568; margin: 30px 0; }
        ...
        .weather-card .ranking-link { color: #3b82f6; text-decoration: none; font-size: 1.1em; } /* Tailwindの色に合わせる */
        .weather-card .ranking-link:hover { text-decoration: underline; }
        .weather-card .excuse-text { font-size: 0.9em; text-align: center; margin-top: 5px; padding: 0 10px; color: inherit; opacity: 0.8; }
    </style>
</x-app-layout>